    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_date_time_picker:
                DateTimePickerDialog.create(this, Calendar.getInstance(), calendar -> ToastUtil.show(DateUtil.formatDateTime(calendar.getTime()))).show();
                break;
            case R.id.btn_waiting:
                WaitingDialog.show(this, "Title", "loading...", true, true);
                break;
            case R.id.btn_year_moth_picker:
                YearMonthPickerDialog.create(this, Calendar.getInstance(), (view, year, month, dayOfMonth) -> ToastUtil.show(year + "年" + month + "月" + dayOfMonth + "日")).setFixTitle("选择年月").show();
                break;
